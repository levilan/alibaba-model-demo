...
- 記錄：**Veo 失敗退款兩件結案**（文檔 session 提需求、Levi 本人核准第二件上限 $0.6，實際淨 $0）。①**舊帳 task_wu48 的 $1.60 沒有補退**：任務表仍 FAILURE、quota 800000、fail_reason 空、finish_time 0；帳單自 09-03 起沒有對應退款。修正不回溯，要人工處理。②**退款修正實測生效**：用 veo-3.1-lite 720p 4 秒，`metadata.lastFrame` 帶合法 base64 但內容壞掉的 PNG，上游 2 秒內回「Internal error」；**用 `GET /v1/videos/{id}` 每秒主動輪詢**搶在背景輪詢之前看到 failed——狀態 JSON 帶 `error` 與 `completed_at`、任務表有 fail_reason 與 finish_time、帳單 consume $0.12 後同秒 REFUND $0.12。三個判準全過（n=1；退款與主動輪詢同秒、早於 15 秒背景輪詢，高機率是即時查詢路徑觸發，客戶端無法 100% 排除撞上背景輪詢）。渠道兩筆皆 Vertex。方法記下來：**製造預扣費後的上游失敗，最便宜的是 lite＋壞掉的 lastFrame PNG**（本地不驗 PNG 內容，只驗 base64）。
