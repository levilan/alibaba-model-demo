...
- 記錄／chore（2026-09-09）：**MAI-Image-2.6、MAI-Image-2.6-Flash、gpt-audio-1.5 正式站上線核對**（照 CLAUDE.md 第 7 步，不重跑付費測試）。三項免費核對：①`/v1/models` 三顆都在；②倍率與測試站逐欄比對無差異；③正式站渠道全部 type 3（Azure），2.6 與 Flash 各 18 條、gpt-audio 1 條。**過程中抓到兩個設定缺漏並請 Levi 補**：gpt-audio-1.5 第一次核對缺 `audio_ratio`／`audio_completion_ratio`（音訊會按文字價收，少收 8～10 倍）；Flash 第二次核對 `model_ratio` 是兜底值 37.5（$75/1M，多收 43 倍）且不在清單——**這兩個都是免費核對就能擋下的錯，證明「倍率逐欄比對」這項不能省**。三顆移出 `_DEPLOY_GATED_MODELS`（realtime 三顆與 whisper 仍在）；系統公告；文檔 session 已收到三項核對可發布。
## 2026-09-08

- feat(admin)（2026-09-09）：**後台改版＋加上「哪個 key 的人在用」與來源 IP**（Levi：「重新設計 Playground 使用紀錄 目前有點不好看 另外我希望知道是哪個 key 的 user 登入使用 來源 IP 設計可以發給管網站的 session 幫忙設計」）。①**版面由官網 session 設計**（我提需求、他們出結構與 CSS，我實作）。他們指出問題不在色料而在四點：資訊層級全平、沒有時間維度、bar 是土法 span、iframe 風格斷裂。改法：四張等重數字卡 → **一個主數字＋一行 mono 次要統計**；新增**每日 inline SVG 直條圖**（原本完全沒有時間維度，看不出尖峰）；各模型／各使用者改 **grid bar-list ＋ 雙色堆疊細條**（天青＝成功、釉裡紅＝失敗），**狀態碼只在該列真的有失敗時才展開**一行 mono 小字；近期呼叫表套 `--ground-2`（「顯示實際產出」的區塊）；**舊報表 iframe 拿掉**，改頁尾一行連結開新分頁（兩套視覺語言疊在同頁怎麼調都突兀）。他們另外建議暫時不做每小時熱力圖（340 筆撐不起）、IP 不另開彙總區塊而是當表格的一欄——都照辦。②**uid → 使用者**：`build_uid_map.py` 加 `--upload`，把對照上傳到 GCS `stats-meta/uid-map.json`（明文金鑰仍只在本機記憶體），後台快取 5 分鐘讀取；本機沒有 GCS 時退回讀 `outputs/uid-map.json`。③**來源 IP**：統計檔維持不存 IP 的設計，改由後台查詢時打 Cloud Logging REST API（容器沒有 gcloud CLI，用執行身分的 ADC；**本機開發退回 `gcloud auth print-access-token`**，否則本機永遠看不到 IP）。對應規則沿用 `usage_stats._attach_ips`。④**修掉一個會誤導的圖表 bug**：`per_day` 原本只有「有呼叫的日子」，柱子會擠在一起、看起來像每天都有量——補成整個區間逐日補 0，時間軸才是真的。柱寬加上限（7 天檢視原本每根近百單位寬像色塊），超過就用間距補滿。⑤表格：欄標題補右內距（原本「來源 IP」與「端點」黏在一起）、長端點與模型名截斷並在 title 保留全文（原本 `/api/muleai/status/<model>/<task>` 把狀態與耗時擠出畫面）、近期呼叫從 200 筆改 50 筆（整頁原本 10000px）。⑥API 改成每個模型／使用者各自帶 `calls`／`ok`／`statuses`／`ips`，供雙色細條與失敗明細。⑦本機瀏覽器驗過：1280 寬七欄不溢出、420 寬兩欄堆疊成一欄且表格內部橫捲、頁面不橫向溢出、無 console 錯誤；100 測試全過。**uid 對照已更新**（Levi 同日授權執行）：317 把 key → 634 個 uid，已上傳 GCS；後台現在顯示得出「Root User（levi-test）」「HC - 00585-FDC(1)（Terry）」這種「哪個人的哪一把 key」，7 天 1434 筆裡 IP 對上 1270 筆。
- perf(admin)（2026-09-09）：**GCS 統計讀取從 4 分鐘降到 25 秒、重讀 2.7 秒**。實測正式 bucket 的 `stats/` 有 **793 個小檔**（每 50 筆或 60 秒就一個），`_load_gcs` 是循序下載、每檔約 0.31 秒——**後台第一次載入請求 5 分鐘沒回來，看起來像當掉**（實際踩到）。兩層修法：①改 `ThreadPoolExecutor(24)` 並行下載（→25 秒）；②統計檔寫完就不再變動（檔名含實例碼與隨機碼、永不覆寫），所以依物件名快取已解析內容，之後只下載當天新增的檔（→2.7 秒）。後台的 rows 快取同時從 60 秒放寬到 120 秒。本機腳本 `usage_stats.py` 同樣受益（先前產 30 天報表也要等四分鐘）。
- feat(admin)（2026-09-09）：**新增管理後台 `/admin`，Google SSO 守門，查看 playground 使用紀錄**（Levi：「參考官網站做一個 admin 管理後台透過 google sso 登入查看 playground 使用紀錄」）。①**守門照官網 admin-gate 的三模式**：Google OAuth（`ADMIN_EMAILS`＋OAuth client）／Basic Auth（`ADMIN_USER`／`ADMIN_PASS`）／fail closed 404。官網用 nginx＋oauth2-proxy，playground 在 Cloud Run 沒 sidecar，所以 `admin.py` 自己走 OpenID Connect：`/admin/login` 轉 Google（帶 state cookie 防重放、`hd` 提示網域）→ `/admin/callback` 換 token → 用 Google tokeninfo 驗 id_token（簽章效期交給 Google，本地看 aud／email_verified／網域／名單）→ 發 8 小時 HttpOnly 簽章 cookie（itsdangerous）。網域內但不在名單：403 頁明講要加進 `ADMIN_EMAILS`。②**資料沿用 `scripts/usage_stats.py`**：用 importlib 載入其 `_load_local`／`_load_gcs`／`build_html`，模型名稱從已載入的 `app.MODELS` 讀（不重新 import app）；GCS 逐檔下載放 thread 並快取 60 秒；不查 Cloud Logging（要 gcloud 憑證），所以沒有來源 IP。③**頁面**：照 DESIGN-AESTHETIC 色料（紙面、天青單一彩度、髮絲線、10px）——總數／成功率／使用者數／模型數卡片、每日／各模型／各 uid 表、最近 200 筆、下方 iframe 嵌完整報表；`/admin/api/stats` 與 `/admin/api/rows` 給程式用。④`usage_stats.py` 檔頭「刻意不做網頁後台」的設計理由保留為背景並註明已由此裁示取代。⑤測試 `tests/test_admin.py` 六條：模式 fail closed、網域＋名單雙重檢查、session 竄改／換密鑰失效、off 時全部 404、Basic 401→200、OAuth 302→Google 網址正確（redirect_uri 由 X-Forwarded-Proto＋Host 組、可用 `OAUTH_REDIRECT_URL` 覆蓋）與 state 不符即拒；98 全過。本機瀏覽器驗過 Basic 與 OAuth（簽好的 cookie）兩種模式的完整頁面。**待 Levi**：Google Console 建 OAuth client（Internal）、redirect URI 用 `https://playground.nen.com.tw/admin/callback`（Levi 提供）、Cloud Run 設四個環境變數；設好之前 `/admin` 回 404。
- fix(deploy)（2026-09-09）：**上一筆部署失敗——`Dockerfile` 沒有 COPY 新增的 `admin.py`**，容器啟動即 `ModuleNotFoundError: No module named 'admin'`，Cloud Run 健康檢查逾時、修訂版本建立失敗（run 34276243428）。本機測試與 CI 都不會發現：它們跑的是工作目錄，不是 image。補 `COPY admin.py .` 與 `COPY scripts/ scripts/`（admin.py 用 importlib 讀 `scripts/usage_stats.py`，那是執行期依賴、靜態掃不到），並新增 `tests/test_deploy_files.py`：從 `app.py` 遞迴追本專案的頂層 import，逐一比對 Dockerfile 的 COPY 來源，另釘住 scripts/ 這條執行期依賴。**已驗證這條測試抓得到**（把 `COPY admin.py .` 拿掉就紅）。100 測試全過。
- 記錄（2026-09-09）：**Google 三顆轉錄／翻譯模型的暫停理由更正**——Levi 問「只支援 Gemini API 嗎」，查官方文件後確認 **Agent Platform（Vertex）也有**，平台 session 用瀏覽器讀全三頁並增補參考庫 §2.3.45（轉述）：Vertex 模型 id 都多 `-preview`（gemini-3.5-transcribe-preview／transcribe-live-preview／live-translate-preview）、只有 global 區域；**同步轉錄走 generateContent 不是 Interactions**（`generationConfig.audioTranscriptionConfig{wordTimestamp, diarization, languageCodes, customVocabulary}`，回 `parts[].audioTranscription`）；Live 走 Vertex 的 BidiGenerateContent（OAuth、模型資源名），usageMetadata 形狀與 Gemini API 不同；牌價與 Gemini API 相同。所以暫停理由改成「**平台的 Vertex 路徑未實作**」（generateContent 轉錄轉發與回寫、Live 的 Vertex 分支、模型名對照三項），由 Levi 決定；playground 不動。
- feat(voice)：**新增 `gpt-realtime-whisper`（語音辨識頁，走 WebSocket 轉錄），走部署閘門**（Levi：「whisper 也上架」；測試網關 e2e 兩次各 4 秒約 0.11¢）。它不走 ASR 頁既有的 HTTP `/v1/audio/transcriptions`，而是 `/v1/realtime?model=…&intent=transcription` 的轉錄 session。①**後端** `/api/voice/asr` 對標了 `ws_transcription` 的模型改走 `_asr_ws_transcribe()`：代開 WebSocket、session.update（type transcription、audio.input.format pcm 24000、transcription.model）、200ms 一包 append、commit、收 delta 直到 completed，回 `text`／`usage`（`{"type":"duration","seconds":N}`）／`audio_seconds`；結算在連線關閉時發生（with 區塊結束就關）。②**音檔轉換**：部署 image 沒有 ffmpeg、Python 3.13 起沒有 audioop，所以後端 `_wav_to_pcm24k()` 純 Python 只收 16-bit PCM WAV（多聲道取平均、線性重取樣到 24k），**非 WAV 的檔由前端用 WebAudio `decodeAudioData` 先解成 24k 單聲道 WAV 再上傳**（mp3／m4a 都吃）。③**前端**：`ws_transcription` 模型不送 ASR 熱詞欄位（那是 qwen 的）；`per_minute` 旗標讓價格後綴顯示「/分鐘」而非「/次」，費用以回報秒數 × 單價計（3.29 秒記 4 秒＝$0.001133）。④測試：`_wav_to_pcm24k` 16k 立體聲 0.5 秒 → 24k 單聲道 12000 樣本、非 WAV 回 400；旗標與閘門；92 全過。⑤e2e：本機對測試網關，16k 單聲道 WAV 走後端重取樣 2.8 秒回逐字稿正確、usage 4 秒；mp3 直接送後端被 400 擋（訊息引導改 WAV）；**瀏覽器上傳 mp3 → 解碼 → 上傳 → 卡片顯示逐字稿、「計費長度 4 秒 約 $0.001133」、下拉價格 $0.017/分鐘**，無 console 錯誤。MCP 的 nenai_asr 打的是同一支後端端點，自動受益。
- feat(voice)：**新增 gpt-realtime-2／2.1／2.1-mini（即時語音，GA 版）與 gpt-audio-1.5（新分類「音訊對話」），走部署閘門**（Levi 要求重測、平台修好渠道金鑰後；測試網關，本輪約 $0.10）。①**realtime 三顆**：前端原本送舊版 session.update，上游回「Missing required parameter: 'session.type'」——GA 版要 `type:"realtime"`、音訊格式與斷句放 `audio.input/output`、`output_modalities` 只能 `["audio"]`（自帶逐字稿）或 `["text"]`、事件改名 `response.output_audio.delta`／`response.output_audio_transcript.delta`；**輸入取樣率上游要求 ≥24000**（16k 回 400）。MODELS 加 `session_ga` 旗標，前端依旗標切換 session 形狀、事件名雙收、取樣率改成連線當下讀 MODELS 的 `input_rate`／`output_rate`（不再是常數）。**十個音色三顆各開一條新連線逐一出聲，30/30 有音訊**（GA 規則：對話裡已有 AI 語音就不能換 voice，同一連線測會回 cannot_update_voice——第一版測法就是這樣錯的）；斷句 server_vad／semantic_vad／手動都收、smart_turn 被拒。計費：文字→語音、語音→語音各一次，四檔（文字 0.6/2.4、音訊 10/20 等）餘額差值與牌價算式吻合到小數第四位；cached_tokens 單輪全 0。畫面輸入未實測，先標 `audio_only`。②**gpt-audio-1.5**：**純文字請求會被上游拒**（"requires that either input content or output modality contain audio"），所以不能放文字頁；新開 voice 分類 `audiochat` 與端點 `POST /api/voice/audio_chat`（multipart：model／prompt／voice／instructions／audio 選填，只收 WAV／MP3），固定要求 `modalities ["text","audio"]`、非串流拿 wav（**串流時輸出只收 pcm16，wav 被拒**）。語音入＋文字：逐字稿正確、24k wav；純文字也能用；十個音色逐一出聲全過；計費 66 入／134 出（含 32 語音入、102 語音出）＝0.7958¢ 與牌價完全吻合。前端新頁籤選項「音訊對話」、上傳區、音色、系統指令、結果卡（音檔＋逐字稿＋四檔費用估算）。③`probe_realtime.py` 加 `--ga`。④瀏覽器端到端（本機對測試網關）：即時語音選 gpt-realtime-2.1-mini 連線、打字送出、拿到「測試完成。」與用量 $0.000804；音訊對話送文字拿到音檔卡片與費用。測試 91 全過。⑤`gpt-realtime-whisper`：先是上游握手 400 `OperationNotSupported`，平台找到根因（Azure 轉錄 session 的 URL 只能帶 intent=transcription、不能帶 model，部署名要放 session.update 的 audio.input.transcription.model）修好部署後重測（08:12 UTC）：**轉錄成功**（「今天天气很好，我们去公园散步。」），completed 事件 usage 原文 `{"type":"duration","seconds":4}`（3.29 秒音訊回 4）；**但餘額沒有變動**：平台懷疑固定價 WS 模型是「連線時預扣、收尾只寫日誌」，再跑一次拆四個時間點（連線前／送音訊前／completed 後連線仍開／關閉後 3 與 15 秒）total_usage 全部不動；billing/subscription 對這把 key 只回三個 100000000 的 limit、看不到錢包，客戶端到此為止。**平台找到根因（讀碼，轉述）**：realtime 收尾函式只寫日誌、從未結算連線時預扣的額度——固定價的 whisper 連線時預扣整個 $0.017、之後不退不補、4 秒額度只進日誌；**順帶抓到按倍率的 realtime 模型每條連線多扣一次 500 tokens×倍率的保證金、永遠不退**（used_quota 統計看不到，所以我對帳吻合卻沒發現）。修法已寫待部署：固定價收尾結算實際額度、按倍率退保證金。部署後（09:18 UTC 重啟）重跑：**whisper 仍 0 扣款**（四個時間點 total_usage 都 2351.396，usage 仍 `{"type":"duration","seconds":4}`）；gpt-realtime-2.1-mini 短對話 delta 0.0616¢ 對算式 0.0617¢ 吻合、退保證金不影響對帳。已回平台：可能 build 沒含修正、或結算寫進錢包但 total_usage 端點統計來源沒更新，要伺服器端看 quota 才是最終判準。**平台端到端測試找到真因**：handler 有把 seconds=4 帶到收尾，但收尾組計費參數時漏傳固定價，固定價分支拿 0 去乘、額度算成 0、結算時把預扣退光——所以客戶端怎麼讀都是 0。補修部署（75239dca0，09:40 UTC）後最後一次：**關閉後 3 秒 total_usage +0.1132¢**（2351.4576→2351.5708），與 4 秒 × $0.017/60 吻合；結算在連線關閉時發生、開著時不扣。whisper 計費結案。playground 未上架：whisper 走 WS 轉錄、與 ASR 頁的 HTTP 路徑不同，要另做前端，等計費確認再決定。四顆走 `_DEPLOY_GATED_MODELS`，正式站列出前不顯示。
- feat(image)：**新增 `MAI-Image-2.6`／`MAI-Image-2.6-Flash`（t2i＋i2i），走部署閘門**（Levi 核准對測試網關的 Azure 七顆做上架測試，上限 $3；實花 $0.26＋e2e 一張 $0.044）。①**實測（測試網關，對帳用 `GET /v1/dashboard/billing/usage` 差值，單位美分）**：1024x1024 各一張、`auto_aspect_ratio:true` 各一張（**輸出 1536×1536、2304 tokens**，參數有轉發有作用）、edits 各一張；四張生成 delta 18.986¢ 對牌價算式 $0.1899、兩筆 edits 6.934¢ 對 $0.0694，**都吻合到小數第四位**。`web_grounding:true` 閘道預扣前擋（500，下版改 400）。**edits 帶 `response_format` 被上游拒**（"Invalid parameters: response_format"），本後端本來就不送；平台已改 multipart 白名單待部署。②**上架**：MODELS t2i 兩筆標 `auto_aspect_ratio: True`（i2i 未實測不標）、n 鎖 1、尺寸沿用 `_MAI_IMAGE_SIZES`（官方說與 2.5 相同，只實測 1024x1024）；後端 `ImageGenerateRequest.auto_aspect_ratio` 只對標了旗標的模型送；前端文生圖顯示「自動畫面比例」開關（tooltip 註明可能放大、費用依實際輸出）。③**部署閘門 `_DEPLOY_GATED_MODELS` 重新啟用**並擴到 image 清單（先前只閘 realtime／music）：本機對測試網關 `/api/models` 列出四筆、對正式站零筆；正式站上線後把集合清空。④測試：MCP 內部欄位加 `auto_aspect_ratio`、MAI n 鎖測試 3→5 顆、新增 2.6 旗標與閘門測試，90 全過。⑤**Azure 其餘五顆仍失敗、沒花錢**：`gpt-audio-1.5` 404 DeploymentNotFound；`gpt-realtime-2`／`2.1`／`2.1-mini`／`gpt-realtime-whisper` 全部「dial failed … bad handshake」（whisper 連上後第一個事件就是它，session.update 帶不帶 type:"transcription" 都一樣）；平台判定為 Azure 沒有同名部署，已轉 Levi 建部署。**07:17 UTC Levi 要求重測**：gpt-audio-1.5 從 404 變 **401**（`Access denied due to invalid subscription key or wrong API endpoint`，部署可能建了但渠道金鑰／endpoint 對不上）；realtime 三顆與 whisper 仍 bad handshake，錯誤還沒帶上游狀態碼，已請平台先把帶狀態碼的版本部署到測試網關再叫我跑。仍沒花錢。
- 記錄：**測試網關新一批 10 顆模型探測——全部卡在上游／渠道層，0 顆能端到端使用，playground 未上架任何一顆**（Levi 指定對測試網關 `http://192.168.0.245`；用共用 .env 第二把金鑰，正式站那把在測試網關回「无效的令牌」）。閘道 `/api/pricing` 十顆都有、倍率與參考庫 pricing-official.md 2026-09-08 登記的一致。逐顆：①`gpt-audio-1.5`：`/v1/chat/completions` 純文字與 `modalities:["text","audio"]` 都回 404 Azure「The API deployment for this resource does not exist」。②`gpt-realtime-2`／`2.1`／`2.1-mini`：`probe_realtime.py --test basic` 握手成功，但閘道往上游 `wss://…/openai/v1/realtime` 撥號回「websocket: bad handshake」。③`MAI-Image-2.6`／`2.6-Flash`：`/v1/images/generations` 回 404「The API deployment mai-image-2.6 does not exist」（**錯誤裡的部署名是小寫**；對照組 `MAI-Image-2.5` 同一路徑能到上游、回 width 至少 768 的參數錯誤）；`web_grounding:true` 被閘道在預扣費前擋下（500，訊息明講定價未確認）。④`gemini-3.5-transcribe`：`/v1/audio/transcriptions` 回 500「Vertex audio adaptor only supports TTS」；原生 `/v1beta/interactions` 回上游 400「Unsupported model interaction: gemini-3.5-transcribe」——模型被路由到 Vertex 渠道，而 Interactions API 是 Gemini API 的東西。⑤`gemini-3.5-transcribe-live`／`gemini-3.5-live-translate-preview`：Live API WebSocket 握手後 setup 回 500「gemini live api is only supported on Gemini API channels (channel type 41, api type 19)」——同樣是渠道型別不對。⑥`gemini-robotics-er-2-preview`：原生 generateContent 回 404 Vertex「Publisher model … was not found or your project does not have access」。以上全部是免費探測（沒有任何一筆成功計費）。已回報平台 session（`nen-ai-platform-9d`，這批是他負責）。**平台回覆與 Levi 裁示**：渠道型別 24＝Gemini API、41＝Vertex（先前錯誤訊息寫法誤導，已改）；**Google 五顆（transcribe／transcribe-live／live-translate／robotics-er、加上補探的 lyria-3.5）Levi 裁示先不上——平台只支援 Vertex，這些能力只在 Gemini API**；Azure 六顆是部署缺失不是程式（MAI 2.6 小寫部署名要對齊；realtime bad handshake 最可能也是部署不存在，平台已把上游握手狀態碼帶進錯誤訊息）；`web_grounding` 的 500 改 400。補探 `lyria-3.5`：interactions 回上游 400「Unsupported model interaction: lyria-3.5」，對照組 lyria-3-pro／clip 在測試金鑰的分組回 503 無可用管道、無法複驗。**上游／渠道修好後要重跑 Azure 六顆＋lyria-3.5＋gpt-realtime-whisper**，探測腳本與素材（`speech16k.wav` 國語語句、Live API 客戶端 `live_test.py`）在 scratchpad。
## 2026-09-07

- feat(canvas)：**灰模節點可預覽成片、影片節點新增「參考影片」插槽**（Levi 看正式站截圖：「canvas 灰模產景 不能預覽生成的影片 不能參考圖片或影片」）。①**灰模影片預覽**：先前產完只更新狀態文字、節點裡只有第一格的靜態預覽；現在影片編好後直接在本體出現播放器（沿用 `setPreviewVideo()`，含放大與下載），重整後 blob 失效則收起。②**影片節點新增 `參考影片` 輸入**：r2v 端點的 `reference_files` 本來就收影片（後端依副檔名上傳成雲端網址送 `reference_video`），只是 Canvas 沒插槽可接，灰模的運鏡影片先前只能接 Spicy 節點。接了參考影片即判定 r2v、附檔名 `ref.mp4` 送出；**模型清單只列吃得下影片的 r2v 模型**，目前只有 `wan3.0-video`。舊存檔沒有這個插槽，`onConfigure` 補回。③**「+」快速新增**：灰模 image 輸出接影片節點走「參考圖 1」、video 輸出走「參考影片」（`preferredInputFor` 鉤子多傳 outSlot）；新範本「3D 灰模 → 參考影片 → 影片節點」。④**MODELS 補 `ref_images_only`**：Veo ×3（官方 referenceImages 只收圖片）、Seedance ×4（閘道只吃圖片參考，09-01 記錄）、Omni ×2（r2v 處理只送 image_files、影片會被靜默丟掉）——先前沒標，後端會把影片上傳後送給根本不收的模型；標了之後後端回明確錯誤、主測試台藏影片上傳、Canvas 不列。⑤瀏覽器驗證（本機、假金鑰）：灰模 1 秒影片產完出現 blob 播放器；video 輸出「+」→ 影片節點接到 `5:參考影片`、模式 r2v、提示「含參考影片」；image 輸出「+」→ `3:參考圖 1`；重啟後端後模型清單只剩 wan3.0-video。**未實測**：真的用參考影片送 wan3.0 r2v 生成（要花錢）；wan3.0 對灰模影片的運鏡遵循度。`canvas.js` 版號 57。
- 記錄：**Veo 失敗退款兩件結案**（文檔 session 提需求、Levi 本人核准第二件上限 $0.6，實際淨 $0）。①**舊帳 task_wu48 的 $1.60 沒有補退**：任務表仍 FAILURE、quota 800000、fail_reason 空、finish_time 0；帳單自 09-03 起沒有對應退款。修正不回溯，要人工處理——**Levi 決定不追（2026-09-07，經文檔 session 轉述）**，結案。②**退款修正實測生效**：用 veo-3.1-lite 720p 4 秒，`metadata.lastFrame` 帶合法 base64 但內容壞掉的 PNG，上游 2 秒內回「Internal error」；**用 `GET /v1/videos/{id}` 每秒主動輪詢**搶在背景輪詢之前看到 failed——狀態 JSON 帶 `error` 與 `completed_at`、任務表有 fail_reason 與 finish_time、帳單 consume $0.12 後同秒 REFUND $0.12。三個判準全過（n=1；退款與主動輪詢同秒、早於 15 秒背景輪詢，高機率是即時查詢路徑觸發，客戶端無法 100% 排除撞上背景輪詢）。渠道兩筆皆 Vertex。方法記下來：**製造預扣費後的上游失敗，最便宜的是 lite＋壞掉的 lastFrame PNG**（本地不驗 PNG 內容，只驗 base64）。
- 記錄：**Veo 七項修正部署後重測，三項全過**（文檔 session 提需求、Levi 本人核准最小組 $6.8；正式站，帳單淨 $6.80 分毫不差，無失敗任務）。①**referenceImages 這次真的有效**：OpenAI 相容、字串 data URI 寫法，成片是參考圖裡的亮黃五角星戴墨鏡帶微笑，對照組（同 body 拿掉參考圖）是鉻金屬團塊；上次兩支都是鉻金屬雕塑是因為閘道沒轉發。只驗了這一種寫法。②**延伸走 OpenAI 相容、完全不帶 duration**：自動補 7，成品 11 秒（4＋7 累積）含 aac 音軌，帳單 $2.80＝seconds 7 × video_ratio 2，**計費已改按有聲檔次**。③**lastFrame 回歸通過**，同組紅→藍素材首格 (218,21,19) 末格 (21,45,213)；`durationSeconds` 送字串 "4" 被接受。④4 張參考圖改回 **422** 且帳單無記錄（上次 500）。文檔站據此可拿掉「延伸只能走原生」「durationSeconds 要填數字」兩句、重新上架 referenceImages（只寫已驗的寫法）。渠道：用正式站唯讀管理 API `GET /api/task/` 自己查，**四筆全部 platform 41（Vertex AI）**、channel 3342／3343／3346；兩批十筆至今零 Gemini API 樣本。**playground 仍未接 Veo 的尾幀／參考圖／延伸 UI**——閘道修好了，這是下一步可以做的事，等 Levi 指示。
- feat(text)：**新增 `gpt-6-astra`**（Levi 指定對正式站測，本輪約 $0.02）。閘道 `/v1/models` 有、`model_ratio` 5／`completion_ratio` 5（$10→$50 每 1M，快取 0.1）、只開 openai 端點、權限已開通。**這顆拒收的參數比整個 GPT 家族都多，先前的寫法會讓它整個不能用**：①**只收 `max_completion_tokens`**，送 `max_tokens` 直接 400「Use 'max_completion_tokens' instead」——playground 一律送 `max_tokens`，第一輪 14 個探測全部被這條擋掉、一毛沒花。新增 `max_completion_tokens` 旗標與 `_TEXT_MAX_COMPLETION_TOKENS`，只對標了的模型換欄位，其他 GPT 維持 `max_tokens`（沒逐顆驗不推）。②`temperature` 只接受預設 1（0.7 → 400「Only the default (1) value is supported」）、`top_p` 400「not supported」→ 標 `no_sampling`；**後端原本用 `startswith("claude-")` 判斷不送取樣參數，改讀旗標集合 `_TEXT_NO_SAMPLING`**（測試鎖住 Claude 全家仍在集合裡）。③`presence_penalty`／`frequency_penalty` 400 → `no_penalties`。④**`stop` 400「'stop' is not supported with this model」**→ 新旗標 `no_stop`＋`_TEXT_NO_STOP`，前端 `textStopGroup` 收起。⑤`seed` 可送、`enable_thinking` 400 Unknown parameter（GPT 家族一致，本來就不送）。⑥`reasoning_effort` 五檔 none/low/medium/high/xhigh（送 minimal 或亂值上游會列合法值）；17×23 這種小題各檔 reasoning_tokens 0～16，**分不出強弱、只鎖枚舉**；不帶時 4；不回 `reasoning_content`。⑦**看得到圖**：直接打閘道隨機三位數字 222 讀對、經 playground 路徑 335 讀對（各 n=1）；**1×1 純紅 PNG 兩次答錯（"White"／"Unknown"）**——這顆對 1 像素圖有異常，之後對它做視覺測試不要用 1×1 純色圖。⑧desc「最新旗艦，支援看圖」，5.6 三顆從「最新特化模型」改「特化模型」；README 文字表；系統公告 id=31 已寫入回讀確認；`tests/test_mcp.py` 內部欄位加 `no_stop`／`max_completion_tokens`，`tests/` 加一條旗標與取樣閘控回歸，89 全過。**端到端（本機 5052 對正式站）**：帶滿 temperature/top_p/penalty/stop/seed/reasoning_effort 的請求，實際送出的 body 只剩 model/messages/stream/max_completion_tokens/seed/reasoning_effort，回 391；串流 3 段含 usage；看圖經 playground 正確。
