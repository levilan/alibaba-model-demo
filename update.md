...
- docs：補齊 `gemini-3.5-flash-lite` 與 `gemini-2.5-flash-lite` 的 `thinkingConfig` 對照（文檔站要的資料）。兩者思考都**預設關閉**，但對不合法設定的反應不同：`thinkingBudget: 128` 在 2.5-lite 回 400 並說明範圍，在 3.5-lite **靜默收下但完全不思考**；單獨送 `includeThoughts` 亦同（2.5-lite 400、3.5-lite 靜默無效）。實務門檻約在 512。我們一律送 `thinkingBudget: -1`，不受這個陷阱影響。另確認 `thinkingBudget` 同樣是傾向而非上限（512→157、2048→170）。README 補上完整對照表。
