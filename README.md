...
| 影片 | 依連接的圖片組合自動切換 t2v／i2v（首尾幀）／r2v（參考圖或參考影片；接參考影片時只列吃得下影片的模型） |
| 圖像編輯 | 需連接一張來源圖片 |
...
| 標記 | 在圖片上畫筆刷／箭頭／編號；輸出不含記號的原圖，記號自動轉成方位文字接在下游提示詞前面（可選擇把記號畫進圖片） |
...
**Spicy 影片模型走專屬模板**（`w3.0-video-*`、`wan2.7-i2v-spicy`）：不是自由改寫，而是把使用者
那句話填進固定模板，依**實際放了什麼素材**選模板——沒放圖＝文生影片版、放了首幀＝圖生影片版、
首尾都放＝首尾幀過渡版。目前選的秒數與畫面比例會一併填進模板。放了首幀（或首尾幀）時，**那些圖會一起交給改寫模型看**，
所以模板裡「要保持一致的元素」寫的是來源圖真正有的東西，而不是籠統一句「與來源相同」。參考素材模式與圖片類模型
（Z-Image、圖像編輯、換臉）沿用通用改寫。

三份模板**可以在後台 `/admin` 直接編輯**（頁面下方「提示優化模板」）：每份各有「儲存」與
「還原預設」，存檔後約一分鐘內全面生效。改過的內容存在 GCS（`prompt-templates/spicy.json`，
可用 `SPICY_TEMPLATES_KEY` 改路徑），沒設定 GCS 時退回本機 `outputs/spicy-templates.json`。
儲存層讀不到或壞掉一律退回程式內建的預設模板，不會讓提示優化整個失效。
...
可查詢的呼叫紀錄（時間區間、使用者、模型、成功／失敗、關鍵字，每次 50 筆可續載）。資料就是
`scripts/usage_stats.py` 讀的那份統計（GCS 的 `stats/*.jsonl`，沒雲端時讀本機 `outputs/stats/`）。
版面由官網 session 設計（2026-09-09）。**點每日圖的柱子**會把查詢時間設成那一天，**點模型或
使用者那一列**會只看它，再點一次取消；下拉選單與這兩張表永遠同步。
