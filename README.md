...
**剪貼簿貼上圖片（2026-08-17）**：在主測試台的任何分頁按 Ctrl+V／⌘V，剪貼簿裡的圖片會自動放進當前頁面的圖片欄位（`static/js/app.js` 的 `_PASTE_TARGETS` 路由表）：文字分頁的對話圖片（僅視覺模型）、圖片分頁的 i2i 參考圖、影片分頁的首幀→尾幀→參考圖、Spicy 的來源圖／換臉圖、即時對話的畫面附件（純語音模型不收）、音樂的靈感圖片。目標區會閃一下光暈並跳 toast 說明貼到了哪裡。機制是把檔案塞回原本的 `<input type=file>` 再派發 `change`，與點選檔案完全同一條路。三個刻意的設計：**焦點在文字欄位且剪貼簿同時有文字時不攔**（尊重貼文字的意圖）；**當前頁面沒有可收圖的欄位時安靜略過**（例如 t2i 模式）；單檔欄位已有檔案時讓位給下一個可見空欄（貼首幀、再貼尾幀）。AI Canvas 是獨立頁面，未納入（LiteGraph 自身有節點複製貼上的語意，要另行設計）。
...
> ⚠️ **MAI 三個 t2i 的張數（`n`）鎖 1，長期維持（2026-08-16）**：使用者回報「選 3 張只顯示 1 張、實際計費 3 張」，對測試網關重現——送 `n=3` 時 `data[]` 只回 1 筆 `b64_json`、`usage.num_output_tokens` 只報 1024（一張的量）、沒有任何 metadata 可補圖，但用量增幅 $0.3257 與「3 張 × 1024 tokens × 2.5 × 21.2」完全吻合，也就是**收 3 張的錢、只給 1 張**。回報後閘道端已修掉多收錢的部分（根因：MAI 的 `num_output_tokens` 本來就按實際產出回報，閘道的兜底又把請求的 n 當乘數再乘一遍；現改以上游回報為準）。但（📄 轉述自閘道端）**Azure 上游本來就靜默忽略 `n`、永遠只回 1 張**，所以 `max_n=1`（UI 與後端 `/api/image/generate` 雙層）**不是等修復的暫時措施**——n>1 這個選項對 MAI 從來就不存在。
>
...
**客戶內容預設不上雲。** 政策是「不儲存客戶資料」：即使設定了雲端儲存憑證，生成的
圖片/影片/音訊與上傳的參考素材仍寫入容器本機的 `outputs/` 目錄（無狀態環境下實例
回收即消失——這是刻意的，不是缺陷）；憑證只供**使用者統計** jsonl 使用（統計不含
prompt、生成結果與 IP，見「使用者統計」）。要恢復「客戶內容上雲、回簽名網址」的
行為，需明確設 `STORE_OUTPUTS=true`。

設定以下任一組環境變數即可啟用雲端物件儲存後端（統計一定會用；客戶內容僅在
`STORE_OUTPUTS=true` 時使用，簽名網址有效期 7 天）：
