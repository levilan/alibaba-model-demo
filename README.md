...
> | `512` | **8/8** | **4/8** |
>
> 需要多步推導的題目每次都思考、`thoughtsTokenCount` 也明顯較高（210～369）；看起來很短的陷阱題則時常被判斷成不需思考（144～180）。**所以給固定預算時，同一個設定在不同 prompt 上表現不同**，不能當成「開了思考」的保證。
>
> **只有 `thinkingBudget: -1` 是可靠的，而且它不等於「一個很大的固定預算」**——實測連 `2+2` 這種一定不需要推理的題目，`-1` 也是 8/8 都思考（106～128 tokens），而固定預算在陷阱題上只有 3～4/8。兩個型號、兩種題目共 32 次全部命中。我們平台的「思考開」送的就是 `-1`，不受這個問題影響。
>
