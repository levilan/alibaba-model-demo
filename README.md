...
>
> ⚠️ **自訂尺寸走的是 `size` 字串，不是 `width`/`height` 兩個欄位。** 實測正式環境送 `{"width":2000,"height":2000}`（400 萬像素，若被讀取必定超限報錯）卻正常產出 1024×1024——證明頂層 `width`/`height` 在這條路徑上會被**靜默丟棄**。閘道端已修（改從 `Extra` map 取值，並刻意不宣告在 `dto.ImageRequest`，否則會把 `width`/`height` 透傳給 dall-e 這類只認 `size` 的上游而讓原本能跑的請求變 400），但那個修復還在 feature branch、只部署到測試網關 `192.168.0.245`，正式環境尚未生效。等它上線後 `width`/`height` 也會通，但**沒有必要改**——`size` 這條現在就能達成同樣的事。
