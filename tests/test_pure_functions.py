...
# 2026-08-15 由本專案直接對照截圖逐格核對，18 格全數相符。先前是二手轉述自
# nen-ai-platform 的核對結果，現在兩邊獨立對過同一份官方頁面、結論一致。
#
# 這份截圖**沒有**確認到的兩件事，不要當成它背書過：
#   1. 計價單位。官方表的單位欄寫的是「/ 1 count」，沒有出現 second 字樣。我們
#      （與閘道）都按**每秒**計算，依據是閘道端的計費公式與這些數字對得上已知的
#      每秒牌價，不是這份截圖。
#   2. Lite 有沒有 4K 檔次。截圖在 Lite 的 1080p 那列就被截斷了。我們表上 Lite 的
#      4K 沿用 1080P 價，依據是閘道的行為（4k 落到 1080p 檔），同樣不是官方頁面。
#
# 這條測試失敗時該怎麼辦（三種情況，都不是刪掉它）：
#   1. 官方調價 → 改期望值，並更新上面那個日期
#   2. 我們某一格核對錯了 → **回頭比對官方頁面**再改，不要照著實際扣款反推
#      （實際扣款可能正處在某個修正尚未部署的落差裡，見上面第 3 點）
