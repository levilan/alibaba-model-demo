...
(function () {
    ...
    // ── 一鍵執行整張圖 ──────────────────────────────────────────────────────────
    // 每個節點都有自己的「▶ 生成」按鈕，但一條 5 個節點的鏈要按 5 次、還得自己記
    // 順序（上游沒生成完，下游按下去只會拿到 null 而失敗）。這裡依拓樸順序自動跑完。
    //
    // 三個刻意的設計決定：
    //
    // 1. **不執行文字節點。** 它的「生成文字」是 opt-in 的（面板上就寫著「不生成也
    //    可直接當純文字輸出」），Run All 去呼叫會把使用者打好的 prompt 換成 LLM
    //    生成的內容——那是破壞性的，而且從畫面上看不出來被換掉了。文字節點本來就
    //    透過 onExecute 把文字往下游送，不執行它不影響整條鏈。想要生成過的文字，
    //    先按那個節點自己的按鈕，Run All 之後就會沿用 generatedText。
    // 2. **已經有結果的節點略過。** Run All 最常見的用途是「跑完整條鏈」與「失敗後
    //    重跑」，每次都全部重跑等於重複付費。要重跑單一節點就按它自己的按鈕。
    // 3. **上游失敗就跳過下游。** 上游沒有產出時，下游拿到的是 null，硬跑只會多一次
    //    註定失敗的呼叫（影片模型那是真金白銀）。
    let _runAllInFlight = false;

    function _runnableNodes() {
        return (graph._nodes || []).filter(n => typeof n.generate === 'function');
    }
    function _nodeHasResult(node) {
        return !!(node.imageUrl || node.videoUrl || node.audioUrl || node.resultUrl);
    }
    // 直接上游節點（只看有連線的輸入，不看有沒有資料——資料要等執行完才有）
    function _upstreamOf(node) {
        const out = [];
        (node.inputs || []).forEach((inp, i) => {
            const n = node.getInputNode(i);
            if (n) out.push(n);
        });
        return out;
    }
    // 拓樸排序：回傳 { order, cyclic }。有環時把還沒排進去的節點原樣附在後面並回報，
    // 不要靜默丟掉——使用者會以為那些節點被執行過了。
    function _topoSort(nodes) {
        const set = new Set(nodes.map(n => n.id));
        const indeg = new Map(nodes.map(n => [n.id, 0]));
        const nexts = new Map(nodes.map(n => [n.id, []]));
        nodes.forEach(n => {
            _upstreamOf(n).forEach(u => {
                if (!set.has(u.id)) return;          // 上游不是可執行節點（例如文字節點）就不算依賴
                indeg.set(n.id, indeg.get(n.id) + 1);
                nexts.get(u.id).push(n.id);
            });
        });
    ...
    async function runAll() {
        if (_runAllInFlight) { showToast('已經在執行中'); return; }

        const runnable = _runnableNodes();
        if (!runnable.length) { showToast('畫布上沒有可執行的節點'); return; }

        const pending = runnable.filter(n => !_nodeHasResult(n));
        const skipped = runnable.length - pending.length;
        if (!pending.length) {
            showToast(`${runnable.length} 個節點都已經有結果了。要重跑請按節點自己的按鈕`);
            return;
        }

        const { order, cyclic } = _topoSort(pending);
        if (cyclic.length) {
            showToast(`偵測到循環連線，有 ${cyclic.length} 個節點無法排序，已排除`);
        }

        // 送出前確認。刻意把「有幾個影片節點」單獨列出來——影片是非同步任務、單價最高、
        // 而且一旦送出就無法取消，這是使用者最需要在按下去之前知道的一件事。
        // 用註冊型別判斷，不要用「有沒有 videoUrl 屬性」那種間接特徵
        const VIDEO_TYPES = ['nenai/video', 'nenai/video_edit', 'nenai/video_animate'];
        const videoCount = order.filter(n => VIDEO_TYPES.includes(n.type)).length;
        const textNodes = (graph._nodes || []).filter(n => typeof n.generateText === 'function').length;
        const lines = [`即將依順序執行 ${order.length} 個節點。`];
        if (videoCount) lines.push(`其中 ${videoCount} 個是影片節點——影片任務送出後無法取消。`);
        if (skipped) lines.push(`已有結果的 ${skipped} 個節點會略過（要重跑請按節點自己的按鈕）。`);
        if (textNodes) lines.push(`${textNodes} 個文字節點不會重新生成，會沿用目前的內容。`);
        lines.push('', '確定要開始嗎？');
        ...
        let ok = 0, skippedByUpstream = 0;
        try {
            for (let i = 0; i < order.length; i++) {
                const node = order[i];
                // 上游失敗就別浪費一次註定失敗的呼叫
                const brokenUpstream = _upstreamOf(node).some(u => failed.has(u.id));
                if (brokenUpstream) {
                    failed.add(node.id);
                    skippedByUpstream++;
                    if (node.statusEl) node.statusEl.textContent = '略過（上游未完成）';
                    continue;
                }
                showToast(`執行中 ${i + 1}/${order.length}：${node.title}`);
                // 把畫面捲到正在執行的節點，長鏈才看得出進度
                try { lgCanvas.centerOnNode(node); lgCanvas.setDirty(true, true); } catch (e) { /* 非關鍵 */ }
                const statusBefore = node.statusEl ? node.statusEl.textContent : null;
                try {
                    await node.generate();
                } catch (e) {
                    failed.add(node.id);
                    if (node.statusEl) node.statusEl.textContent = '錯誤：' + e.message;
                    continue;
                }
                // generate() 內部是自己 catch 錯誤的（不會往外拋），所以「有沒有成功」
                // 要看它有沒有產出結果，不能只靠沒拋例外來判斷
                if (_nodeHasResult(node)) { ok++; continue; }
                failed.add(node.id);
                // 必填欄位沒填時，各節點是 showToast() 之後直接 return、**不會**寫自己的
                // 狀態列（例如「請輸入 prompt」）。而 toast 只有一個、會被下一個節點的
                // 訊息蓋掉，於是最後的總結說「失敗原因寫在各節點上」就成了假話。
                // 這裡補一個 fallback，確保每個失敗的節點上都留得下痕跡。
                if (node.statusEl && node.statusEl.textContent === statusBefore) {
                    node.statusEl.textContent = '未執行（必填欄位未完成）';
                }
            }
        } finally {
        ...
        const parts = [`完成 ${ok} 個`];
        const realFailed = failed.size - skippedByUpstream;
        if (realFailed > 0) parts.push(`失敗 ${realFailed} 個`);
        if (skippedByUpstream) parts.push(`因上游未完成而略過 ${skippedByUpstream} 個`);
        showToast(parts.join('、') + '。失敗原因寫在各節點上');
    }
