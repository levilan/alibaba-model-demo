...
- **realtime（即時語音）已通**（2026-08-16，閘道端補完阿里 adaptor 的 realtime 分支）：`qwen3.5-omni-plus-realtime` 對正式環境驗證上架；`qwen3.5-omni-flash-realtime`、`qwen-audio-3.0-realtime-plus`、`qwen-audio-3.0-realtime-flash` 對測試網關（192.168.0.245）驗證上架。**這三個正式環境尚未部署**——部署後要補做：①系統公告（`scripts/update_announcements.py`）②通知文檔 session ③確認 `/api/pricing` 四檔倍率有出現。快照版 `qwen3.5-omni-flash-realtime-2026-03-15` 經使用者裁示不上架。探測工具：`scripts/probe_realtime.py`。
- **Lyria 三個音樂模型（2026-08-17 上架）同樣只在測試網關**，正式部署後同上要補公告與文檔通知（固定價 clip $0.04／pro $0.08／002 $0.06，`/api/pricing` 的 `quota_type=1` 直接可用，不需人工快照）。
...
- **realtime 兩個計費落差：video 已修好驗畢、白名單等第二輪重建後再驗**（2026-08-17 回歸對帳）：①`video_tokens` 修正**已生效**（image 輪增幅與官方算式吻合到第四位）。②「有語音輸出時文字輸出仍被計費」第一輪回歸**仍然重現**——閘道端據此查出真因（📄 轉述）：不是部署落差，是**兩本帳**，實際扣款路徑 PreWssConsumeQuota 組 QuotaInfo 時漏帶 AudioOnlyOutputBilling 旗標，白名單只在寫日誌那條路生效；對帳一定要讀扣款側的 `/v1/dashboard/billing/usage`，讀日誌欄金額會看到「文字免費」的假象。已修等重建，**重建後要再跑一輪 basic 對帳**（預期值：flash 0.03243、plus 0.04992），閘道端 session 會來叫。
