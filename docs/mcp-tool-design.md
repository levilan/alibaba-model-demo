...
⚠️ 上線前待辦（文檔站要求，2026-08-25）：各家上游的結果 URL 時效**並不統一**
（DashScope／Veo／本站暫存各異），要嘛逐家實測給出確切數字、要嘛在回應層統一化，
文檔站拒用「約」字上客戶文件——`expires_hint` 必須是實測值。

**計價呈現分工**（與文檔站確認，2026-08-25）：`list_models` 回**即時實際單價**
（資料源 `/api/pricing`，與 playground UI 的價格顯示同源）——agent 需要在行動前回答
「這要花多少錢」。文檔站的「不放金額」慣例不受影響：文件只描述欄位形狀，
數字以工具回傳為準——「文件不印價格、工具回即時價格」兩者並立。

**文件引用**：文檔站有機器可讀介面（`docs.nen.com.tw/en/llms.txt` 索引＋
`llms.mdx/<路徑>.md` 純 markdown 單頁）。工具 description 與錯誤訊息**不要內嵌長文件**，
指向對應的 llms.mdx 頁即可；`list_models` 可考慮每模型附 `doc_url`。

**Spicy 的文檔面**：比照現行規範——公開文件站**完全不提**（含 `?spicy=1` 參數本身），
與該組模型不進公開文件的既有政策一致；有需要的客戶走既有的個別管道。
